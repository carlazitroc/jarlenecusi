...
$(function(){
	...
	var rrType = 'permission';

	var $root = $(selector);

	var $listgroup = $root.find('.table tbody');
	var permissionTemplateStr = $root.find('script[data-template=row]').text();
	...
	var selectRoleForm = function(data){
		var sourceUrl = '<?php echo site_url('user/admin/permission')?>';
			...
			$.getJSON(sourceUrl+'/search.json?id='+this.ids.join(','), function(rst){
				if(rst.data){
					$(rst.data).each(function(idx, _row){
						...
						row.end_date = null;
						row.value = 1;
						row.permission_id = _row.id;
						row.permission_name = _row.name;
	...
	var editRoleForm = function(formData, callback){
		...
		var $elm = $root.find('[hc-dialog-template="permission-form"]').clone();
	...
	var fetch = function(){
		...
		if(hc.editor.id){
			...
			$.getJSON('<?php echo $endpoint_url_prefix?>/'+hc.editor.id+'/permission/search.json', function(rst){
