...
$list.recordList({
	...
	onRowAction: function(action, elm){
		...
		if( action == 'tag_filter'){
			var queries =  Fragment.parseQuery(location.hash.length > 1 ? location.hash : location.search);
			var newTagStr = 'tag:'+data.id;
			var currentSearch = typeof queries.q !='undefined' && queries.q ? queries.q : '';
			if(currentSearch.indexOf(newTagStr) ==-1){
				if(currentSearch && currentSearch.length>0) currentSearch+= ' ';
				currentSearch+= newTagStr;
			}
			queries.q = currentSearch;
			History.pushState(null,docTitle,apiURLPrefix + '?'+Fragment.buildQuery(queries));
		}
		if( action == 'category_filter'){
			var queries =  Fragment.parseQuery(location.hash.length > 1 ? location.hash : location.search);
			var newTagStr = 'category:'+data.id;
			var currentSearch = typeof queries.q !='undefined' && queries.q ? queries.q : '';
			if(currentSearch.indexOf(newTagStr) ==-1){
				if(currentSearch && currentSearch.length>0) currentSearch+= ' ';
				currentSearch+= newTagStr;
